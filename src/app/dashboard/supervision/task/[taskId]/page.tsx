...
export default async function GlobalTaskDetailPage({ params, searchParams }: { params: { taskId: string }, searchParams: { evidenceId?: string } }) {
    const effectiveUserId = await getActiveBusinessId();
    if (!effectiveUserId) return redirect('/dashboard');

    const data = await getTaskDetails(params.taskId, searchParams.evidenceId, effectiveUserId);

...
export default async function BranchReservationsAnalyticsPage({ params }: { params: { branchSlug: string } }) {
    const context = await getDashboardContext(params.branchSlug)
    if (!context || !context.userId) return redirect('/dashboard')

    const { success, data } = await getReservationsAnalytics(context.userId)

    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-0">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Analítica Local <span className="text-xs bg-white/10 px-2 py-1 rounded-full text-gray-300 align-middle">{context.name}</span></h1>
                    <p className="text-gray-400 mt-2">Métricas y ocupación de la sucursal.</p>
                </div>
            </div>

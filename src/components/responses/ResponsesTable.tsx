...
export default function ResponsesTable({ responses, onViewResponse }: { responses: any[], onViewResponse: (r: any) => void }) {
    ...
    return (
        <div className="w-full">
            {/* Desktop Table View */}
            <div className="hidden md:block overflow-hidden rounded-2xl border border-white/5 bg-[#111]">
                ...
                {responses.map((r) => {
                    ...
                    return (
                        <div
                            key={r.id}
                            onClick={() => onViewResponse(r)}
                            className="bg-[#111] p-4 rounded-2xl border border-white/5 active:bg-white/5 transition"
                        >
                            <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-full ${avatarBg} flex items-center justify-center font-bold text-white shrink-0`}>
                                        {initial}
                                    </div>
                                    <div>
                                        <div className="font-bold text-white">{displayName}</div>
                                        <div className="text-xs text-gray-500">{formatDistanceToNow(new Date(r.createdAt), { addSuffix: true, locale: es })}</div>
                                    </div>
                                </div>
                                <div className="flex gap-0.5">
                                    {[...Array(5)].map((_, i) => (
                                        <Star
                                            key={i}
                                            className={`w-3.5 h-3.5 ${i < ratingVal ? 'text-yellow-500 fill-yellow-500' : 'text-gray-800'}`}
                                        />
                                    ))}
                                </div>
                            </div>
                            ...
                            {r.photo && (
                                <div className="mb-3 h-32 w-full rounded-xl overflow-hidden border border-white/5 relative">
                                    <img
                                        src={r.photo}
                                        alt="Evidencia"

...
import { Search, MoreHorizontal, Edit, Trash2, Shield, ExternalLink, Loader2 } from 'lucide-react'
import { updateTenantPlan } from '@/actions/admin' // We'll create this next
                            ...
                            <tr key={tenant.id} className="hover:bg-white/5 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <select
                                        disabled={isPending}
                                        value={tenant.plan}
                                        onChange={(e) => handlePlanChange(tenant.userId, e.target.value)}
                                        className={cn(
                                            "bg-transparent border border-white/10 rounded px-2 py-1 text-xs font-bold focus:bg-black focus:border-violet-500 outline-none cursor-pointer",
